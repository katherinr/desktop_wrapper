...
void CSendToSound::Update(const SOUND_FUNC_INPUT& sound_inp, const SOUND_FUNC_SETTINGS& sound_set )
{
	...
	double     lg_down_time = sound_set.lg_down_time;
	double       lg_up_time = sound_set.lg_up_time;
	...
	{
		sndval_flaps_down = loudness_flaps + 100;
	} // полная громкость (100), зацикленный сигнал (+100)
	if (Flaps_curr<Flaps_prev  &&  simulation_status>0)
	{
		sndval_flaps_up = loudness_flaps + 100;
	} // полная громкость (100), зацикленный сигнал (+100)
	...
	{
		sndval_abrks_on = loudness_abrks + 100;
	} // полная громкость (100), зацикленный сигнал (+100)
	if (Abrks_curr<Abrks_prev  &&  simulation_status>0)
	{
		sndval_abrks_off = loudness_abrks + 100;
	} // полная громкость (100), зацикленный сигнал (+100)
	...
	if (lg_up_time  <0.01) { lg_up_time = 0.01; }
	if (lg_down_time<0.01) { lg_down_time = 0.01; }

	double ratelim_lower = -1 / lg_up_time;
	double ratelim_upper = 1 / lg_down_time;

	double LG_ltd = LG_ratelim.Update(ratelim_lower, ratelim_upper, (double)LG_curr);
	...
	{
		sndval_lg_down = loudness_lg + 100;
	} // полная громкость (100), зацикленный сигнал (+100)
	if (LG_ltd<LG_prev  &&  simulation_status>0)
	{
		sndval_lg_up = loudness_lg + 100;
	} // полная громкость (100), зацикленный сигнал (+100)
